...
namespace VRTK.SecondaryControllerGrabActions
{
    ...
    [
        AddComponentMenu(
            "VRTK/Scripts/Interactions/Interactables/Secondary Controller Grab Actions/VRTK_ControlDirectionGrabAction")
    ]
    public class VRTK_ControlDirectionGrabAction : VRTK_BaseGrabAction
    {
        [
            Tooltip(
                "The distance the secondary controller must move away from the original grab position before the secondary controller auto ungrabs the object.")
        ]
        public float ungrabDistance = 1f;

        [
            Tooltip(
                "The speed in which the object will snap back to it's original rotation when the secondary controller stops grabbing it. `0` for instant snap, `infinity` for no snap back.")
        ]
        public float releaseSnapSpeed = 0.1f;

        [
            Tooltip(
                "Prevent the secondary controller rotating the grabbed object through it's z-axis.")
        ]
        ...
        public override void Initialise(
            ...
            VRTK_InteractGrab currentSecondaryGrabbingObject,
            Transform primaryGrabPoint,
            Transform secondaryGrabPoint
        )
        {
            
            base.Initialise(currentGrabbdObject,
            currentPrimaryGrabbingObject,
            currentSecondaryGrabbingObject,
            primaryGrabPoint,
            secondaryGrabPoint);
        ...
        protected virtual IEnumerator RealignOnRelease()
        {
            ...
            while (elapsedTime < releaseSnapSpeed)
            {
                transform.localRotation =
                    Quaternion
                        .Lerp(releaseRotation,
                        initialRotation,
                        (elapsedTime / releaseSnapSpeed));
        ...
        protected virtual void AimObject()
        {
            ...
            else
            {
                transform.rotation =
                    Quaternion
                        .LookRotation(secondaryGrabbingObject
                            .transform
                            .position -
                        primaryGrabbingObject.transform.position,
                        secondaryGrabbingObject
                            .transform
            ...
            {
                transform
                    .Translate(primaryGrabbingObject
                        .controllerAttachPoint
                        .transform
                        .position -
                    primaryInitialGrabPoint.position,
                    Space.World);
            }
        }
        ...
        protected virtual void ZLockedAim()
        {
            Vector3 forward =
                (
                secondaryGrabbingObject.transform.position -
                primaryGrabbingObject.transform.position
                ).normalized;

            // calculate rightLocked rotation
            Quaternion rightLocked =
                Quaternion
                    .LookRotation(forward,
                    Vector3
                        .Cross(-primaryGrabbingObject.transform.right, forward)
                        .normalized);

            // delta from current rotation to the rightLocked rotation
            Quaternion rightLockedDelta =
                Quaternion.Inverse(grabbedObject.transform.rotation) *
                rightLocked;
            ...
            rightLockedDelta
                .ToAngleAxis(out rightLockedAngle, out rightLockedAxis);
            ...
            Quaternion upLocked =
                Quaternion
                    .LookRotation(forward,
                    primaryGrabbingObject.transform.forward);

            // delta from current rotation to the upLocked rotation
            Quaternion upLockedDelta =
                Quaternion.Inverse(grabbedObject.transform.rotation) * upLocked;
            ...
            grabbedObject.transform.rotation =
                (upLockedAngle < rightLockedAngle ? upLocked : rightLocked);
        }
    }
}
