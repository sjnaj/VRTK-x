...
namespace VRTK
{
    ...
    public class VRTK_MoveInPlace : MonoBehaviour
    {
        ...
        [
            Tooltip(
                "If this is checked then the left controller engage button will be enabled to move the play area.")
        ]
        public bool leftController = true;

        [
            Tooltip(
                "If this is checked then the right controller engage button will be enabled to move the play area.")
        ]
        public bool rightController = true;

        [Tooltip("The button to press to activate the movement.")]
        public VRTK_ControllerEvents.ButtonAlias
            engageButton = VRTK_ControllerEvents.ButtonAlias.TouchpadPress;

        [Tooltip("The device to determine the movement paramters from.")]
        public ControlOptions
            controlOptions = ControlOptions.HeadsetAndControllers;

        [
            Tooltip(
                "The method in which to determine the direction of forward movement.")
        ]
        ...
        [
            Tooltip(
                "The maximun speed in game units. (If 0 or less, max speed is uncapped)")
        ]
        public float maxSpeed = 4;

        [
            Tooltip(
                "The speed in which the play area slows down to a complete stop when the engage button is released. This deceleration effect can ease any motion sickness that may be suffered.")
        ]
        public float deceleration = 0.1f;

        [
            Tooltip(
                "The speed in which the play area slows down to a complete stop when falling is occuring.")
        ]
        ...
        [
            Tooltip(
                "The degree threshold that all tracked objects (controllers, headset) must be within to change direction when using the Smart Decoupling Direction Method.")
        ]
        ...
        [
            Tooltip(
                "The maximum amount of movement required to register in the virtual world.  Decreasing this will increase acceleration, and vice versa.")
        ]
        ...
        [
            Tooltip(
                "An optional Body Physics script to check for potential collisions in the moving direction. If any potential collision is found then the move will not take place. This can help reduce collision tunnelling.")
        ]
        ...
        protected Dictionary<Transform, List<float>>
            movementList = new Dictionary<Transform, List<float>>();

        protected Dictionary<Transform, float>
        ...
        public virtual void SetControlOptions(
        ...
        )
        {
            ...
            if (
                ...
                (
                controlOptions == ControlOptions.HeadsetAndControllers ||
                controlOptions == ControlOptions.ControllersOnly
                )
            )
            {
                VRTK_SharedMethods
                    .AddListValue(trackedObjects,
                    VRTK_DeviceFinder
                        .GetActualController(controllerLeftHand)
                        .transform,
                    true);
                VRTK_SharedMethods
                    .AddListValue(trackedObjects,
            ...
            if (
                ...
                (
                controlOptions == ControlOptions.HeadsetAndControllers ||
                controlOptions == ControlOptions.HeadsetOnly
                )
            )
            {
                VRTK_SharedMethods
                    .AddListValue(trackedObjects, headset.transform, true);
            }
        }
        ...
        protected virtual void OnEnable()
        {
            ...
            bodyPhysics =
                ...
                bodyPhysics != null
                    ? bodyPhysics
                    : FindObjectOfType<VRTK_BodyPhysics>()
                );
            ...
            for (int i = 0; i < trackedObjects.Count; i++)
            {
                ...
                VRTK_SharedMethods
                    .AddDictionaryValue(movementList,
                    trackedObj,
                    new List<float>(),
                    true);
                VRTK_SharedMethods
                    .AddDictionaryValue(previousYPositions,
                    trackedObj,
                    trackedObj.transform.localPosition.y,
                    true);
            }
        }
        ...
        {
            CheckControllerState(controllerLeftHand,
            leftController,
            ref leftSubscribed,
            ref previousLeftControllerState);
            CheckControllerState(controllerRightHand,
            rightController,
            ref rightSubscribed,
            ref previousRightControllerState);
            previousEngageButton = engageButton;
        }
        ...
        protected virtual void FixedUpdate()
        {
            ...
            if (MovementActivated() && !currentlyFalling)
            {
                ...
                float speed =
                    Mathf
                        .Clamp((
                        (speedScale * 350) *
                        (CalculateListAverage() / trackedObjects.Count)
                        ),
                        0f,
                        maxSpeed);
            ...
            {
                currentSpeed -=
                    (currentlyFalling ? fallingDeceleration : deceleration);
            }
            else
            {
                currentSpeed = 0f;
        ...
        protected virtual bool MovementActivated()
        {
            return (
            movementEngaged ||
            engageButton == VRTK_ControllerEvents.ButtonAlias.Undefined
            );
        }

        protected virtual void CheckControllerState(
            GameObject controller,
            bool controllerState,
            ref bool subscribedState,
            ref bool previousState
        )
        {
            if (
                controllerState != previousState ||
                engageButton != previousEngageButton
            )
            {
                SetControllerListeners(controller,
                controllerState,
                ref subscribedState);
            }
        ...
        protected virtual float CalculateListAverage()
        {
            ...
            for (int i = 0; i < trackedObjects.Count; i++)
            {
                ...
                float previousYPosition =
                    VRTK_SharedMethods
                        .GetDictionaryValue(previousYPositions, trackedObj);
                float deltaYPostion =
                    Mathf
                        .Abs(previousYPosition -
                        trackedObj.transform.localPosition.y);

                // Convenience code.
                List<float> trackedObjList =
                    VRTK_SharedMethods
                        .GetDictionaryValue(movementList,
                        trackedObj,
                        new List<float>(),
                        true);
        ...
        protected virtual Vector3 HeadsetPosition()
        {
            return (
            headset != null
                ? new Vector3(headset.forward.x, 0, headset.forward.z)
                : Vector3.zero
            );
        }
        ...
        protected virtual Vector3 SetDirection()
        {
            switch (directionMethod)
            {
                ...
                case DirectionalMethod.ControllerRotation:
                    return CalculateControllerRotationDirection(DetermineAverageControllerRotation(
                    ) *
                    Vector3.forward);
                case DirectionalMethod.LeftControllerRotationOnly:
                    return CalculateControllerRotationDirection((
                    controllerLeftHand != null
                        ? controllerLeftHand.transform.rotation
                        : Quaternion.identity
                    ) *
                    Vector3.forward);
                ...
                case DirectionalMethod.EngageControllerRotationOnly:
                    ...
                    engagedController != null
                        ? engagedController.scriptAlias.transform.rotation
                        : Quaternion.identity
                    ) *
                    Vector3.forward);
        ...
        protected virtual Vector3 CalculateCouplingDirection()
        {
            ...
            if (directionMethod == DirectionalMethod.SmartDecoupling)
            {
                ...
                float curXDir =
                    (headset != null ? headset.rotation.eulerAngles.y : 0f);
                ...
                closeEnough =
                    ...
                    Mathf
                        .Abs(curXDir -
                        controllerLeftHand.transform.rotation.eulerAngles.y) <=
                    smartDecoupleThreshold
                    );
                closeEnough =
                    closeEnough &&
                    (
                    Mathf
                        .Abs(curXDir -
                        controllerRightHand.transform.rotation.eulerAngles.y) <=
                    smartDecoupleThreshold
                    );
        ...
        CalculateControllerRotationDirection(
            Vector3 calculatedControllerDirection
        )
        {
            return (
            Vector3.Angle(previousDirection, calculatedControllerDirection) <=
            90f
                ? calculatedControllerDirection
                : previousDirection
            );
        }
        ...
        protected virtual void MovePlayArea(
        ...
        )
        {
            ...
            if (playArea != null)
            {
                Vector3 finalPosition =
                    new Vector3(movement.x + playArea.position.x,
                        playArea.position.y,
                        movement.z + playArea.position.z);

                // Debug.Log(playArea.position.ToString());
        ...
        CanMove(
            VRTK_BodyPhysics givenBodyPhysics,
        ...
        )
        {
            ...
            Vector3 proposedDirection =
                (proposedPosition - currentPosition).normalized;
            float distance =
                Vector3.Distance(currentPosition, proposedPosition);
            return !givenBodyPhysics
                .SweepCollision(proposedDirection, distance);
        }
        ...
        protected virtual void EngageButtonPressed(
            object sender,
        ...
        protected virtual void EngageButtonReleased(
            object sender,
            ControllerInteractionEventArgs e
        )
        {
        ...
        protected virtual Quaternion DetermineAverageControllerRotation()
        {
            ...
            {
                newRotation =
                    AverageRotation(controllerLeftHand.transform.rotation,
                    controllerRightHand.transform.rotation);
        ...
        protected virtual Quaternion
        AverageRotation(Quaternion rot1, Quaternion rot2)
        {
            return Quaternion.Slerp(rot1, rot2, 0.5f);
        }

        protected virtual void SetControllerListeners(
            GameObject controller,
            bool controllerState,
            ref bool subscribedState,
        ...
        )
        {
            if (controller != null)
            {
                ...
                ToggleControllerListeners(controller,
                toggleState,
                ref subscribedState);
            }
        }

        protected virtual void ToggleControllerListeners(
            GameObject controller,
            bool toggle,
            ref bool subscribed
        )
        {
            ...
            if (controllerEvents != null)
            {
                ...
                {
                    controllerEvents
                        .UnsubscribeToButtonAliasEvent(previousEngageButton,
                        true,
                        EngageButtonPressed);
                    controllerEvents
                        .UnsubscribeToButtonAliasEvent(previousEngageButton,
                ...
                {
                    controllerEvents
                        .SubscribeToButtonAliasEvent(engageButton,
                        true,
                        EngageButtonPressed);
                    controllerEvents
                        .SubscribeToButtonAliasEvent(engageButton,
                ...
                {
                    controllerEvents
                        .UnsubscribeToButtonAliasEvent(engageButton,
                        true,
                        EngageButtonPressed);
                    controllerEvents
                        .UnsubscribeToButtonAliasEvent(engageButton,
