...
namespace VRTK
{
    ...
    public delegate void
        BodyPhysicsEventHandler(object sender, BodyPhysicsEventArgs e);
    ...
    public class VRTK_BodyPhysics : VRTK_DestinationMarker
    {
        ...
        [
            Tooltip(
                "If checked then the body collider and rigidbody will be used to check for rigidbody collisions.")
        ]
        public bool enableBodyCollisions = true;

        [
            Tooltip(
                "If this is checked then any items that are grabbed with the controller will not collide with the body collider. This is very useful if the user is required to grab and wield objects because if the collider was active they would bounce off the collider.")
        ]
        public bool ignoreGrabbedCollisions = true;

        [
            Tooltip(
                "An array of GameObjects that will not collide with the body collider.")
        ]
        public GameObject[] ignoreCollisionsWith;

        [
            Tooltip(
                "The collider which is created for the user is set at a height from the user's headset position. If the collider is required to be lower to allow for room between the play area collider and the headset then this offset value will shorten the height of the generated collider.")
        ]
        public float headsetYOffset = 0.2f;

        [
            Tooltip(
                "The amount of movement of the headset between the headset's current position and the current standing position to determine if the user is walking in play space and to ignore the body physics collisions if the movement delta is above this threshold.")
        ]
        public float movementThreshold = 0.0015f;

        [
            Tooltip(
                "The amount of movement of the play area between the play area's current position and the previous position to determine if the user is moving play space.")
        ]
        public float playAreaMovementThreshold = 0.00075f;

        [
            Tooltip(
                "The maximum number of samples to collect of headset position before determining if the current standing position within the play space has changed.")
        ]
        public int standingHistorySamples = 5;

        [
            Tooltip(
                "The `y` distance between the headset and the object being leaned over, if object being leaned over is taller than this threshold then the current standing position won't be updated.")
        ]
        ...
        [
            Tooltip(
                "The maximum height to consider when checking if an object can be stepped upon to.")
        ]
        public float stepUpYOffset = 0.15f;

        [
            Tooltip(
                "The width/depth of the foot collider in relation to the radius of the body collider.")
        ]
        [Range(0.1f, 0.9f)]
        ...
        [
            Tooltip(
                "The distance between the current play area Y position and the new stepped up Y position to consider a valid step up. A higher number can help with juddering on slopes or small increases in collider heights.")
        ]
        ...
        [
            Tooltip(
                "A check to see if the drop to nearest floor should take place. If the selected restrictor is still over the current floor then the drop to nearest floor will not occur. Works well for being able to lean over ledges and look down. Only works for falling down not teleporting up.")
        ]
        public FallingRestrictors
            fallRestriction = FallingRestrictors.NoRestriction;

        [
            Tooltip(
                "When the `y` distance between the floor and the headset exceeds this distance and `Enable Body Collisions` is true then the rigidbody gravity will be used instead of teleport to drop to nearest floor.")
        ]
        ...
        [
            Tooltip(
                "The amount of rounding on the play area Y position to be applied when checking if falling is occuring.")
        ]
        ...
        [
            Tooltip(
                "The VRTK Teleport script to use when snapping to floor. If this is left blank then a Teleport script will need to be applied to the same GameObject.")
        ]
        public VRTK_BasicTeleport teleporter;

        [
            Tooltip(
                "A custom Rigidbody to apply to the play area. If one is not provided, then if an existing rigidbody is found on the play area GameObject it will be used, otherwise a default one will be created.")
        ]
        public Rigidbody customPlayAreaRigidbody = null;

        [
            Tooltip(
                "A GameObject to represent a custom body collider container. It should contain a collider component that will be used for detecting body collisions. If one isn't provided then it will be auto generated.")
        ]
        public GameObject customBodyColliderContainer = null;

        [
            Tooltip(
                "A GameObject to represent a custom foot collider container. It should contain a collider component that will be used for detecting step collisions. If one isn't provided then it will be auto generated.")
        ]
        ...
        protected const string
            BODY_COLLIDER_CONTAINER_NAME = "BodyColliderContainer";

        protected const string
            FOOT_COLLIDER_CONTAINER_NAME = "FootColliderContainer";

        protected bool enableBodyCollisionsStartingValue;

        protected float fallMinTime;

        protected HashSet<GameObject>
            ignoreCollisionsOnGameObjects = new HashSet<GameObject>();
        ...
        public virtual void ApplyBodyVelocity(
            Vector3 velocity,
            bool forcePhysicsOn = false,
            bool applyMomentum = false
        )
        {
        ...
        public virtual void ToggleOnGround(bool state)
        {
            ...
            if (onGround)
            {
                OnStartTouchingGround(SetBodyPhysicsEvent(currentValidFloorObject,
                null));
            }
            else
            {
        ...
        {
            return (
            bodyRigidbody != null ? bodyRigidbody.velocity : Vector3.zero
            );
        }
        ...
        {
            return (
            bodyRigidbody != null ? bodyRigidbody.angularVelocity : Vector3.zero
            );
        }
        ...
        public virtual void ResetIgnoredCollisions()
        {
            ...
            foreach (GameObject
                ignoreCollisionsOnGameObject
                in
                new HashSet<GameObject>(ignoreCollisionsOnGameObjects)
            )
            {
        ...
        public virtual bool SweepCollision(Vector3 direction, float maxDistance)
        {
            ...
            //         .transform
            //         .parent
            //         .TransformPoint(bodyCollider.transform.localPosition +
            //         (bodyCollider.center)) +
            //     (
            //     Vector3.up *
            //     ((bodyCollider.height * 0.5f) - bodyCollider.radius)
            //     );
            Vector3 point1 =
                bodyCollider
                    .transform
            ...
            return VRTK_CustomRaycast
                .CapsuleCast(customRaycast,
                point1,
                point2,
                bodyCollider.radius,
                direction,
        ...
        protected override void OnEnable()
        {
            ...
            SetupPlayArea();
            SetupHeadset();
            footColliderContainerNameCheck =
                VRTK_SharedMethods
                    .GenerateVRTKObjectName(true, FOOT_COLLIDER_CONTAINER_NAME);
        ...
        {
            // Debug.Log(playArea.position.y + "1");
            CheckBodyCollisionsSetting();

            // Debug.Log(playArea.position.y + "2");
            ManageFalling();

            // Debug.Log(playArea.position.y + "3");
            CalculateVelocity();

            // Debug.Log(playAreaVelocity.magnitude);

            if (playAreaVelocity.magnitude > 1f)
            {
            ...
            // Debug.Log(playArea.position.y + "4");
            UpdateCollider();

            // Debug.Log(playArea.position.y + "5");
            // Debug.Log(playArea.position + "" + lastPlayAreaPosition);
            lastPlayAreaPosition =
                (playArea != null ? playArea.position : Vector3.zero);
        }
        ...
        protected virtual void OnDrawGizmos()
        {
            if (drawDebugGizmo && headset != null)
            {
                ...
                Gizmos
                    .DrawSphere(new Vector3(headset.position.x,
                        headset.position.y - 0.3f,
                        headset.position.z),
                    0.075f);
                Gizmos.color = Color.red;
                Gizmos
                    .DrawSphere(new Vector3(currentStandingPosition.x,
                        headset.position.y - 0.3f,
                        currentStandingPosition.y),
                    0.05f);
            }
        }
        ...
        {
            return (
            !VRTK_PlayerObject.IsPlayerObject(checkObject) &&
            (
            !onGround ||
            (
            currentValidFloorObject != null &&
            currentValidFloorObject != checkObject
            )
            )
            );
        }
        ...
        protected virtual bool CheckExistingCollision(GameObject checkObject)
        {
            return (
            currentCollidingObject != null &&
            currentCollidingObject == checkObject
            );
        }
        ...
        protected virtual void SetupPlayArea()
        {
            ...
            if (playArea != null)
            {
                ...
                collisionTracker =
                    playArea.GetComponent<VRTK_CollisionTracker>();
                if (collisionTracker == null)
                {
                    collisionTracker =
                        playArea
                            .gameObject
                            .AddComponent<VRTK_CollisionTracker>();
                }
        ...
        protected virtual void SetupHeadset()
        {
            ...
            if (headset != null)
            {
                currentStandingPosition =
                    new Vector2(headset.position.x, headset.position.z);
            }
        }
        ...
        protected virtual void ManageCollisionListeners(bool state)
        {
            if (collisionTracker != null)
            {
                if (state)
                {
                    collisionTracker.CollisionEnter +=
                        CollisionTracker_CollisionEnter;
                    collisionTracker.CollisionExit +=
                        CollisionTracker_CollisionExit;
                    collisionTracker.TriggerEnter +=
                        CollisionTracker_TriggerEnter;
                    collisionTracker.TriggerExit +=
                        CollisionTracker_TriggerExit;
                }
                else
                {
                    collisionTracker.CollisionEnter -=
        ...
        protected virtual void CollisionTracker_TriggerExit(
            object sender,
        ...
        protected virtual void CollisionTracker_TriggerEnter(
            object sender,
            CollisionTrackerEventArgs e
        )
        {
        ...
        protected virtual void CollisionTracker_CollisionExit(
            object sender,
            CollisionTrackerEventArgs e
        )
        {
        ...
        protected virtual void CollisionTracker_CollisionEnter(
            object sender,
            CollisionTrackerEventArgs e
        )
        {
        ...
        protected virtual BodyPhysicsEventArgs
        SetBodyPhysicsEvent(GameObject target, Collider collider)
        {
        ...
        protected virtual void CalculateVelocity()
        {
            playAreaVelocity =
                (
                playArea != null
                    ? (playArea.position - lastPlayAreaPosition) /
                    Time.fixedDeltaTime
                    : Vector3.zero
                );
        }
        ...
        protected virtual void CheckFalling()
        {
            if (
                isFalling &&
                fallMinTime < Time.time &&
                VRTK_SharedMethods
                    .RoundFloat(lastPlayAreaPosition.y, fallCheckPrecision) ==
                VRTK_SharedMethods
                    .RoundFloat(playArea.position.y, fallCheckPrecision)
            )
            {
                StopFall();
            }
        ...
        protected virtual void SetCurrentStandingPosition()
        {
            if (
                playArea != null &&
                playArea.transform.position != lastPlayAreaPosition
            )
            {
                // Debug
                //     .Log(playArea.transform.position +
                //     "" +
                //     lastPlayAreaPosition);
                Vector3 playareaDifference =
                    playArea.transform.position - lastPlayAreaPosition;
                currentStandingPosition =
                    new Vector2(currentStandingPosition.x +
                        playareaDifference.x,
                        currentStandingPosition.y + playareaDifference.z);
            }
        }
        ...
        protected virtual void SetIsMoving(Vector2 currentHeadsetPosition)
        {
            float moveDistance =
                Vector2
                    .Distance(currentHeadsetPosition, currentStandingPosition);
            float playareaDistance =
                (
                playArea != null
                    ? Vector3
                        .Distance(playArea.transform.position,
                        lastPlayAreaPosition)
                    : 0f
                );
            ...
            if (
                playArea != null &&
                (playareaDistance > playAreaMovementThreshold || !onGround)
            )
            {
                isMoving = false;
            }
        ...
        protected virtual void CheckLean()
        {
            if (bodyCollider != null)
            {
                ...
                Vector3 standingDownRayStartPosition =
                    (
                    headset != null
                        ? new Vector3(currentStandingPosition.x,
                            headset.position.y,
                            currentStandingPosition.y)
                        : Vector3.zero
                    );
                Vector3 rayDirection =
                    (playArea != null ? -playArea.up : Vector3.zero);
                Ray standingDownRay =
                    new Ray(standingDownRayStartPosition, rayDirection);
                RaycastHit standingDownRayCollision;

                //Determine the current valid floor that the user is standing over
                currentValidFloorObject =
                    (
                    VRTK_CustomRaycast
                        .Raycast(customRaycast,
                        standingDownRay,
                        out standingDownRayCollision,
                        defaultIgnoreLayer,
                        Mathf.Infinity,
                        QueryTriggerInteraction.Ignore)
                        ? standingDownRayCollision.collider.gameObject
                        : null
                    );
                ...
                float forwardLength =
                    bodyCollider.radius + leanForwardLengthAddition;
                ...
                {
                    CalculateLean(standingDownRayStartPosition,
                    forwardLength,
                    standingDownRayCollision.distance);
                }
        ...
        protected virtual void CalculateLean(
        ...
        )
        {
            ...
            Vector3 rayDownStartPosition =
                startPosition + (headset.forward * forwardLength);
            rayDownStartPosition =
                new Vector3(rayDownStartPosition.x,
                    startPosition.y,
                    rayDownStartPosition.z);
            ...
            )
            {
                ...
                isMoving =
                    (
                    onGround &&
                    playAreaPositionDelta <= playAreaPositionThreshold &&
                    rayDownDelta > 0f
                        ? true
                        : isMoving
                    );
                ...
                isLeaning =
                    (onGround && rayDownDelta > leanYThreshold ? true : false);

                if (isLeaning)
                {
                    OnStartLeaning(SetBodyPhysicsEvent(downRayCollision
                        .collider
                        .gameObject,
                    downRayCollision.collider));
                }
                else
                {
        ...
        protected virtual void UpdateStandingPosition(
            Vector2 currentHeadsetPosition
        )
        {
            VRTK_SharedMethods.AddListValue (
                standingPositionHistory,
                currentHeadsetPosition
            );
                ...
                if (!isLeaning && currentCollidingObject == null)
                {
                    ...
                    for (int i = 0; i < standingHistorySamples; i++)
                    {
                        float currentDistance =
                            Vector2
                                .Distance(standingPositionHistory[i],
                                standingPositionHistory[standingHistorySamples]);
                        resetStandingPosition =
                            (
                            currentDistance <= movementThreshold
                                ? resetStandingPosition
                                : false
                            );
                    }
        ...
        protected virtual void EnableDropToFloor()
        {
            ...
            teleporter =
                ...
                teleporter != null
                    ? teleporter
                    : FindObjectOfType<VRTK_BasicTeleport>()
                );
            if (teleporter != null)
            {
        ...
        protected virtual void Teleported(
            object sender,
        ...
        protected virtual void EnableBodyPhysics()
        {
            ...
            CreateCollider();
            InitControllerListeners(VRTK_DeviceFinder.GetControllerLeftHand(),
            true);
            InitControllerListeners(VRTK_DeviceFinder.GetControllerRightHand(),
            true);
        }
        ...
        protected virtual void SetupIgnoredCollisions()
        {
            ...
            {
                Collider[] objectColliders =
                    ignoreCollisionsWith[i].GetComponentsInChildren<Collider>();
                for (int j = 0; j < objectColliders.Length; j++)
                {
        ...
        protected virtual void ManagePhysicsCollider(
            Collider collider,
            bool state
        )
        {
        ...
        protected virtual void CheckStepUpCollision(Collision collision)
        {
            if (
                ...
                collision.contacts[0].thisCollider.transform.name ==
                footColliderContainerNameCheck
            )
            {
                ...
                Vector3 colliderWorldCenter =
                    playArea.TransformPoint(footCollider.center);
                Vector3 castStart =
                    new Vector3(colliderWorldCenter.x,
                        colliderWorldCenter.y +
                        (CalculateStepUpYOffset() * stepYIncrement),
                        colliderWorldCenter.z);
                Vector3 castExtents =
                    new Vector3(bodyCollider.radius,
                        boxCastHeight,
                        bodyCollider.radius);
                ...
                if (
                    floorHit &&
                    (floorCheckHit.point.y - playArea.position.y) >
                    stepDropThreshold
                )
                {
                    ...
                    {
                        hitFloorYDelta =
                            playArea.position.y - floorCheckHit.point.y;
                    ...
                    //If there isn't a teleporter then just force the position
                    {
                        playArea.position =
                            new Vector3((
                                floorCheckHit.point.x -
                                (headset.position.x - playArea.position.x)
                                ),
                                floorCheckHit.point.y,
                                (
                                floorCheckHit.point.z -
                                (headset.position.z - playArea.position.z)
                                ));
                    }
                }
            }
        ...
        CreateColliderContainer(string name, Transform parent)
        {
            GameObject generatedContainer =
                new GameObject(VRTK_SharedMethods
                        .GenerateVRTKObjectName(true, name));
            ...
            VRTK_PlayerObject
                .SetPlayerObject(generatedContainer,
                VRTK_PlayerObject.ObjectTypes.Collider);

            return generatedContainer;
        }

        protected virtual GameObject
        InstantiateColliderContainer(
        ...
        )
        {
            ...
            generatedContainer.name =
                VRTK_SharedMethods.GenerateVRTKObjectName(true, name);
            VRTK_PlayerObject
                .SetPlayerObject(generatedContainer,
                VRTK_PlayerObject.ObjectTypes.Collider);
        ...
        protected virtual void GenerateRigidbody()
        {
            if (customPlayAreaRigidbody != null)
            {
                ...
                bodyRigidbody.interpolation =
                    customPlayAreaRigidbody.interpolation;
                bodyRigidbody.collisionDetectionMode =
                    customPlayAreaRigidbody.collisionDetectionMode;
        ...
        protected virtual CapsuleCollider
        GenerateCapsuleCollider(GameObject parent, float setRadius)
        {
            CapsuleCollider foundCollider =
        ...
        protected virtual void GenerateBodyCollider()
        {
            if (bodyColliderContainer == null)
            {
                ...
                {
                    bodyColliderContainer =
                        InstantiateColliderContainer(customBodyColliderContainer,
                        BODY_COLLIDER_CONTAINER_NAME,
                        playArea);
                    bodyCollider =
                        bodyColliderContainer.GetComponent<CapsuleCollider>();
                }
                else
                {
                    bodyColliderContainer =
                        CreateColliderContainer(BODY_COLLIDER_CONTAINER_NAME,
                        playArea);
                    bodyColliderContainer.gameObject.layer =
                        LayerMask.NameToLayer("Ignore Raycast");
                }

                bodyCollider =
                    GenerateCapsuleCollider(bodyColliderContainer, bodyRadius);
            ...
            if (CalculateStepUpYOffset() > 0f)
            {
                if (customFootColliderContainer != null)
                {
                    footColliderContainer =
                        InstantiateColliderContainer(customFootColliderContainer,
                        FOOT_COLLIDER_CONTAINER_NAME,
                        bodyColliderContainer.transform);
                }
                else
                {
                    footColliderContainer =
                        CreateColliderContainer(FOOT_COLLIDER_CONTAINER_NAME,
                        bodyColliderContainer.transform);
                    footColliderContainer.gameObject.layer =
                        LayerMask.NameToLayer("Ignore Raycast");
                }

                footCollider =
                    GenerateCapsuleCollider(footColliderContainer, 0f);
            }
        }
        ...
        protected virtual void CreateCollider()
        {
            ...
            VRTK_PlayerObject
                .SetPlayerObject(playArea.gameObject,
                VRTK_PlayerObject.ObjectTypes.CameraRig);
        ...
        protected virtual void UpdateCollider()
        {
            ...
            {
                float newpresenceColliderYSize =
                    (headset.position.y - playArea.position.y) -
                    (headsetYOffset + CalculateStepUpYOffset());
                float newpresenceColliderYCenter =
                    Mathf
                        .Max((newpresenceColliderYSize * 0.5f) +
                        CalculateStepUpYOffset() +
                        playAreaHeightAdjustment,
                        bodyCollider.radius + playAreaHeightAdjustment);

                bodyCollider.height =
                    Mathf.Max(newpresenceColliderYSize, bodyCollider.radius);
                bodyCollider.center =
                    new Vector3(headset.localPosition.x,
                        newpresenceColliderYCenter,
                        headset.localPosition.z);

                if (footCollider != null)
                {
                    float footThickness =
                        bodyCollider.radius * stepThicknessMultiplier;
                    footCollider.radius = footThickness;
                    footCollider.height = CalculateStepUpYOffset();
                    footCollider.center =
                        new Vector3(headset.localPosition.x,
                            CalculateStepUpYOffset() * 0.5f,
                            headset.localPosition.z);
                }
            }
        }
        ...
        protected virtual void InitControllerListeners(
            GameObject mappedController,
            bool state
        )
        {
            if (mappedController != null)
            {
                IgnoreCollisions(mappedController
                    .GetComponentsInChildren<Collider>(),
                true);

                VRTK_InteractGrab grabbingController =
                    mappedController
                        .GetComponentInChildren<VRTK_InteractGrab>();
                if (grabbingController != null && ignoreGrabbedCollisions)
                {
                    if (state)
                    {
                        grabbingController.ControllerGrabInteractableObject +=
                            new ObjectInteractEventHandler(OnGrabObject);
                        grabbingController.ControllerUngrabInteractableObject +=
                            new ObjectInteractEventHandler(OnUngrabObject);
                    }
                    else
                    {
                        grabbingController.ControllerGrabInteractableObject -=
        ...
        protected virtual IEnumerator RestoreCollisions(GameObject obj)
        {
            ...
            if (obj != null)
            {
                VRTK_InteractableObject objScript =
                    obj.GetComponent<VRTK_InteractableObject>();
                if (objScript != null && !objScript.IsGrabbed())
                {
                    IgnoreCollisions(obj.GetComponentsInChildren<Collider>(),
                    false);
                }
            }
        }

        protected virtual void IgnoreCollisions(
            Collider[] colliders,
            bool state
        )
        {
            if (bodyColliderContainer != null)
            {
                Collider[] playareaColliders =
                    bodyColliderContainer.GetComponentsInChildren<Collider>();
        ...
        protected virtual void OnGrabObject(
            object sender,
        ...
        )
        {
            if (e.target != null)
            {
                ...
                IgnoreCollisions(e.target.GetComponentsInChildren<Collider>(),
                true);
            }
        }

        protected virtual void OnUngrabObject(
            object sender,
            ObjectInteractEventArgs e
        )
        {
            if (
                ...
                playArea.gameObject.activeInHierarchy
            )
            {
                restoreCollisionsRoutine =
        ...
        protected virtual bool FloorIsGrabbedObject(RaycastHit collidedObj)
        {
            ...
            return (
            cachedGrabbedObject != null && cachedGrabbedObject.IsGrabbed()
            );
        }
        ...
        ValidDrop(bool rayHit, RaycastHit rayCollidedWith, float floorY)
        {
            return (
            rayHit &&
            teleporter != null &&
            teleporter
                .ValidLocation(rayCollidedWith.transform,
                rayCollidedWith.point) &&
            !FloorIsGrabbedObject(rayCollidedWith) &&
            FloorHeightChanged(floorY)
            );
        }
        ...
        protected virtual float ControllerHeightCheck(GameObject controllerObj)
        {
            ...
            VRTK_CustomRaycast
                ...
                Mathf.Infinity,
                QueryTriggerInteraction.Ignore);
            return controllerObj.transform.position.y -
            rayCollidedWith.distance;
        }
        ...
        protected virtual bool ControllersStillOverPreviousFloor()
        {
            ...
            bool rightCheck =
                (
                rightController.activeInHierarchy &&
                Mathf.Abs(ControllerHeightCheck(rightController) - previousY) <
                controllerDropThreshold
                );
            bool leftCheck =
                (
                leftController.activeInHierarchy &&
                Mathf.Abs(ControllerHeightCheck(leftController) - previousY) <
                controllerDropThreshold
                );
        ...
        protected virtual void SnapToNearestFloor()
        {
            if (
                !preventSnapToFloor &&
                (enableBodyCollisions || enableTeleport) &&
                headset != null &&
                headset.transform.position.y > playArea.position.y
            )
            {
                ...
                //防止自动上房顶，高度climbThreshold以上的建筑物都不能自动上去
                float climbThreshold = 0.8f;

                hitFloorYDelta = playArea.position.y - rayCollidedWith.point.y;
                if (hitFloorYDelta < -climbThreshold)
                {
                    rayCollidedWith.point =
                        new Vector3(rayCollidedWith.point.x,
                            playArea.position.y,
                            rayCollidedWith.point.z);
                    hitFloorYDelta = 0f;
                }
                ...
                if (
                    ...
                    ValidDrop(rayHit,
                    rayCollidedWith,
                    rayCollidedWith.point.y) ||
                    retogglePhysicsOnCanFall
                    )
                )
                {
        ...
        {
            return (
            hitFloorY < playArea.position.y &&
            ControllersStillOverPreviousFloor()
            );
        }
        ...
        )
        {
            ...
            else
            {
                ...
                if (
                    enableBodyCollisions &&
                    (
                    teleporter == null ||
                    !enableTeleport ||
                    hitFloorYDelta > gravityFallYThreshold
                    )
                )
                {
        ...
        protected virtual void TeleportFall(
            float floorY,
            RaycastHit rayCollidedWith
        )
        {
            ...
            Vector3 newPosition =
                new Vector3(playArea.position.x, floorY, playArea.position.z);
            float originalblinkTransitionSpeed =
                teleporter.blinkTransitionSpeed;
            teleporter.blinkTransitionSpeed =
                (
                Mathf.Abs(hitFloorYDelta) > blinkYThreshold
                    ? originalblinkTransitionSpeed
                    : 0f
                );
            OnDestinationMarkerSet(SetDestinationMarkerEvent(rayCollidedWith
                .distance,
        ...
        protected virtual void ApplyBodyMomentum(bool applyMomentum = false)
        {
            if (applyMomentum)
            {
                ...
                Vector3 appliedMomentum =
                    playAreaVelocity /
                    (rigidBodyMagnitude < 1f ? 1f : rigidBodyMagnitude);
                bodyRigidbody
                    .AddRelativeForce(appliedMomentum,
